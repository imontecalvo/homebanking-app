...
export const newTransfer = async (req, res) => {
  ...
  try {
    ...
    const destUserID = await User.findOne({ where: { username: destUsername },attributes: ["user_id"] });
    if (!destUserID) {
      return res.status(400).json({ msg: "Destionation user not found", ok: false });
    }
    ...
    const destBalance = await Balance.findOne({ where: { user_id: destUserID.user_id, currency } });
